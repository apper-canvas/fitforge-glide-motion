  ...
  return (
    ...
    <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center">
            <div>
                <h1 className="text-2xl font-bold text-text-readable-primary font-display">Profile Settings
                                </h1>
                <p className="text-text-readable-secondary">Manage your fitness preferences and goals
                                </p>
            </div>
            {!editing && <Button variant="primary" size="sm" icon="Edit" onClick={() => setEditing(true)}>Edit
                            </Button>}
        </div>
        {/* Stats Overview */}
        <Card>
        ...
        <Card>
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-text-readable-primary font-display">Fitness Goals
                                </h3>
            </div>
            {editing ? <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {goalOptions.map(goal => <motion.div
                    key={goal.id}
                    whileTap={{
                        scale: 0.98
                    }}
                    ...
                    ${editData.goals?.includes(goal.id) ? "bg-primary/20 border border-primary/30" : "bg-surface-800 hover:bg-surface-700"}
                  `}
                    ...
                    {editData.goals?.includes(goal.id) && <ApperIcon name="Check" size={16} className="text-primary ml-auto" />}
                </motion.div>)}
            </div> : <div className="flex flex-wrap gap-2">
                {profile.goals?.map(goalId => {
                    const goal = goalOptions.find(g => g.id === goalId);

                    return goal ? <Badge key={goalId} variant="primary" icon={goal.icon}>
                        {goal.name}
                    </Badge> : null;
                })}
            </div>}
        ...
        <Card>
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-text-readable-primary font-display">Available Equipment
                                </h3>
            </div>
            {editing ? <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {equipmentOptions.map(equipment => <motion.div
                    key={equipment.id}
                    whileTap={{
                        scale: 0.98
                    }}
                    ...
                    ${editData.equipment?.includes(equipment.id) ? "bg-secondary/20 border border-secondary/30" : "bg-surface-800 hover:bg-surface-700"}
                  `}
                    ...
                    {editData.equipment?.includes(equipment.id) && <ApperIcon name="Check" size={14} className="text-secondary" />}
                </motion.div>)}
            </div> : <div className="flex flex-wrap gap-2">
                {profile.equipment?.map(equipmentId => {
                    const equipment = equipmentOptions.find(e => e.id === equipmentId);

                    return equipment ? <Badge key={equipmentId} variant="secondary" icon={equipment.icon}>
                        {equipment.name}
                    </Badge> : null;
                })}
            </div>}
        ...
        <Card>
            <h3
                className="text-lg font-semibold text-text-readable-primary mb-4 font-display">Workout Schedule
                          </h3>
            {editing ? <div className="space-y-4">
                <div>
                    <label className="block text-sm font-medium text-surface-200 mb-2">Days per week
                                        </label>
                    <div className="grid grid-cols-7 gap-2">
                        {[1, 2, 3, 4, 5, 6, 7].map(days => <Button
                            key={days}
                ...
                <div>
                    <label className="block text-sm font-medium text-surface-200 mb-2">Session duration
                                        </label>
                    <div className="grid grid-cols-4 gap-2">
                        {[30, 45, 60, 90].map(duration => <Button
                            key={duration}
                ...
                <div>
                    <label className="block text-sm font-medium text-surface-200 mb-2">Preferred time
                                        </label>
        ...
        <Card>
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-text-readable-primary font-display">App Theme
                                </h3>
                <Badge variant="primary">
                    {themes[currentTheme].name}
                </Badge>
            </div>
            <p className="text-text-readable-secondary text-sm mb-6">Choose your preferred color theme for the app interface
                          </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.values(themes).map(theme => <motion.div
                    key={theme.id}
                    whileTap={{
                        scale: 0.98
                    }}
                    className={`
                  relative p-4 rounded-lg cursor-pointer transition-all border-2
                  ${currentTheme === theme.id ? "border-primary bg-primary/10" : "border-surface-300 hover:border-surface-400 bg-surface-100"}
                `}
                    ...
                    <div className="flex items-center gap-3 mb-3">
                        ...
                        {currentTheme === theme.id && <ApperIcon name="Check" size={16} className="text-primary ml-auto" />}
                    </div>
                    {/* Theme Info */}
                    <div>
                        <h4 className="font-semibold text-text-readable-primary">
                            {theme.name}
                        </h4>
                    ...
                    <div
                        ...
                        }}>Sample card with {theme.name}theme
                                        </div>
                </motion.div>)}
            </div>
        </Card>
        ...
        {editing && <div className="flex gap-3">
            <Button variant="secondary" size="lg" onClick={handleCancel}>Cancel
                            </Button>
            <Button variant="success" size="lg" fullWidth onClick={handleSave}>Save Changes
                            </Button>
        </div>}
        {/* Danger Zone */}
        <Card variant="error">
            <h3
                className="text-lg font-semibold text-text-readable-primary mb-2 font-display">Reset Profile
                          </h3>
            <p className="text-sm text-surface-950 mb-4">This will reset all your preferences and workout history. This action cannot be undone.
                          </p>
            <Button
                variant="danger"
                size="sm"
                icon="RotateCcw"
                onClick={() => navigate("/setup")}>Reset & Start Over
                          </Button>
        </Card>
    </div>
</div>
